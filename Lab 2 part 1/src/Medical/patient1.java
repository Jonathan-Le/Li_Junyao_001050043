...
//     for(vitalsign a : patient.getPatient())
//        {
        for(vitalsign v : patient.getPatient()){
            System.out.println("\nP="+patient.isPatientNormal(v)+"\n"
                    +v.getAgegroup(v.getAge())+"  "+v.getHeartRate()
                    +v.getWeightPound()+v.getWeightKg()
                    +v.getSystolicBloodPressure()+v.getRespiratoryRate());
